...
Clothes and accessories are supported too, and there is a toggle function implemented, so first tap equips specified clothing item, second tap switches back the original item if there was any.  
# Installation
...
In the Json file the setup for clothes works like this: there should be: ":1" or ":2" after the gear name of the clothing, this specifies which slot the clothing goes. 1 means inner layer (or first accessory slot), 2 means outer layer (and second accessory slot).
